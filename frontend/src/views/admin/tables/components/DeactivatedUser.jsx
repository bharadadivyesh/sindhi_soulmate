...
const DeactivatedUser = () => {
  ...
  const deactivatedUsers = registrationData.filter((items) => items.status === "Deactivated");
  const sortedData = deactivatedUsers.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const handleChange = (listing, e) => {
    const updatedStatus = e.target.value;
    if (updatedStatus !== "") {
      const updatedListing = { ...listing, status: updatedStatus };
      axios.put("http://localhost:3005/put-Registration", updatedListing).then(() => {
  ...
  return (
    <div className="relative overflow-x-auto">
      {token.permissions.find((p) => p.name === "DeActiveted User").permissions
        .fullAccess && (
      ...
      >
        <thead className="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            {token.permissions.find((p) => p.name === "DeActiveted User")
