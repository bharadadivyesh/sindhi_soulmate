...
const totalItems = expiredUsers?.length;
...
const currentItems = expiredUsers?.slice(startIndex, endIndex);
...
const nextPage = () => {
  ...
  return (
    <div className="relative overflow-x-auto">
      ...
      <DownloadTableExcel filename="ExpireSubscription Users" sheet="users" currentTableRef={tableRef.current}>
          <button type="button" className="mb-2 flex items-center rounded-lg bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 px-5 py-2.5 text-center text-sm font-medium text-white me-2 hover:bg-gradient-to-br focus:outline-none focus:ring-4 focus:ring-purple-300 dark:focus:ring-purple-800"><img src={Icon} alt="" className="mr-2 h-4 w-4" />Export</button>
        </DownloadTableExcel>
      </div>
      <table className="w-full text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400" ref={tableRef}>
        <thead className="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th scope="col" className="px-6 py-3 text-xs font-bold tracking-wide text-gray-600">Expire Date</th>
          </tr>
        </thead>
        <tbody>
          {currentItems?.map((items,index)=>{
            return(
              ...
              <th scope="row" className="whitespace-nowrap px-6 py-4 text-sm font-bold text-navy-700 dark:text-white">{new Date(new Date(items.createdAt).getTime() + 5.5 * 60 * 60 * 1000).toLocaleDateString("en-IN", {year: "numeric",month: "numeric",day: "numeric",})}</th>
              ...
              <td className="px-6 py-4 text-sm font-bold text-navy-700 dark:text-white">{new Date(new Date(items.expirationDate).getTime() + 5.5 * 60 * 60 * 1000).toLocaleDateString("en-IN", {year: "numeric",month: "numeric",day: "numeric",})}
              </td>
            </tr>
            )
          })}
