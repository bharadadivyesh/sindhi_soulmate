...
const ViewMembers = () => {
    ...
    const onSubmit = async (data) => {
      if (editItem) {
        await axios.put(`http://localhost:3005/update-roles/${data.email}`, data).then((res) => {
    ...
    const handleDeleteBtnClick = async (item) => {
      await axios.delete(`http://localhost:3005/delete-User/${item.email}`).then((res) => {
          toast.success(res.data.message);
          setRenderComponent(!renderComponent);
        });
    };
        ...
        >
          {(close) => (
            <div className="w-full rounded-lg bg-white p-4 shadow-lg">
              ...
              <h2 className="text-center text-4xl dark:text-white">{couponTitleTOggle === true ? "Update Users" : "Create Users"}</h2>
